...
async function startServer() {
  ...
  // Endpoint to accept image uploads as data URLs and save to uploads folder as WebP
  app.post("/manus-upload", async (req, res) => {
    try {
      ...
      const safeRawName = (fileName || `img_${Date.now()}`).replace(/\.[^/.]+$/, "");
      const safeName = safeRawName.replace(/[^a-zA-Z0-9._-]/g, "_");
      const finalName = `${Date.now()}_${safeName}.webp`;
